...
// FreedomRouter (ERC1967 Proxy)// 常量定义
const FREEDOM_ROUTER = '0x7a2ddD6A265Ab88a0aa8D433137f1e9a8Ad2e01d';
const PANCAKE_ROUTER = '0x10ED43C718714eb63d5aA57B78B54704E256024E';
const WBNB_ADDRESS = '0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c';
const TOKEN_MANAGER_V2 = '0x5c952063c7fc8610FFDB798152D69F0B9550762b';
const HELPER_3 = '0xF251F83e40a78868FcfA3FA4599Dad6494E46034';
...
async function initAfterUnlock() {
  ...
  publicClient = createPublicClient({
    chain: bsc,
    transport: http(rpcUrl || 'https://rpc-not-configured.invalid'),
  });

  await initWalletClients();
  if (config.slippage) {
    $('slippage').value = config.slippage;
    updateSlippageBtn(config.slippage);
  }
...
async function initWalletClients() {
  ...
  for (const wallet of wallets) {
    try {
      ...
      if (isEncrypted(key)) {
        key = await decryptPrivateKey(key);
        if (!key) continue;
      }
      ...
      console.error('初始化钱包失败:', wallet.name, e);
    }
  }
}
...
function renderWalletSelector() {
  ...
  container.querySelectorAll('.wallet-check').forEach((cb) => {
    cb.onchange = () => {
      ...
      if (cb.checked) {
        if (!activeWalletIds.includes(id)) activeWalletIds.push(id);
      } else {
        activeWalletIds = activeWalletIds.filter((aid) => aid !== id);
      }
  ...
  container.querySelectorAll('.wallet-chip').forEach((chip) => {
    chip.onclick = (e) => {
      if (e.target.classList.contains('wallet-check')) return;
      const cb = chip.querySelector('.wallet-check');
...
function updateSelectedCount() {
  $('selectedCount').textContent = activeWalletIds.filter((id) =>
    walletClients.has(id)
  ).length;
}
...
async function loadBalances() {
  try {
    ...
    for (const id of activeWalletIds) {
      ...
      totalBNB += bal;
      balances.push({
        name: wallets.find((w) => w.id === id)?.name || id,
        balance: bal,
        address: wc.account.address,
      });
    }
    $('bnbBalance').textContent = parseFloat(formatUnits(totalBNB, 18)).toFixed(
      4
    );
    $('walletCount').textContent =
      `${activeWalletIds.length}/${wallets.length}`;
    if (balances.length > 0) {
      $('balanceDetails').innerHTML = balances
        .map(
          (b) =>
...
function updateBalanceHint() {
  $('balanceHint').textContent =
    `${activeWalletIds.filter((id) => walletClients.has(id)).length} 个钱包`;
}
...
async function detectToken(addr) {
  if (!addr || !isValidAddress(addr)) {
    ...
    const lpDiv = $('lpInfo');
    if (lpDiv) lpDiv.style.display = 'none';
    const priceDiv = $('priceInfo');
  ...
  try {
    ...
    $('tokenBalanceDisplay').textContent = parseFloat(
      formatUnits(totalBalance, info.decimals)
    ).toFixed(4);
    ...
    if (hasPool) {
      showLPInfo(info);
      showStatus(
        info.isInternal ? 'Four.meme 内盘' : 'PancakeSwap 外盘',
        'success'
      );
    } else {
...
function showLPInfo(info) {
  ...
  if (!div) {
    ...
    div.id = 'lpInfo';
    div.style.cssText =
      'background:#1a1a1a;border:1px solid #333;border-radius:8px;padding:12px;margin-bottom:12px;font-size:12px;';
    document
      .querySelector('.trade-card')
      .insertBefore(div, document.querySelector('.tabs'));
  }
...
function getSellApproveTarget() {
  if (lpInfo.isInternal) {
    return !lpInfo.tmQuote || lpInfo.tmQuote === ZERO_ADDR
      ? TOKEN_MANAGER_V2
      : HELPER3;
  }
...
function getTipRate() {
  const raw =
    config.tipRate != null && config.tipRate !== ''
      ? Number(config.tipRate)
      : DEFAULT_TIP_RATE;
...
async function buy(walletId, tokenAddr, amountStr, gasPrice) {
  ...
  console.log(
    `[BUY] txHash: ${txHash} | 发送耗时: ${((tSent - t0) / 1000).toFixed(2)}s`
  );
  ...
  console.log(
    `[BUY] ✓ 确认 | 等待: ${((tConfirm - tSent) / 1000).toFixed(2)}s | 总计: ${((tConfirm - t0) / 1000).toFixed(2)}s`
  );
  ...
  try {
    ...
    console.warn('[BUY] 自动 approve 失败:', e.message);
  }
...
async function sell(walletId, tokenAddr, amountStr, gasPrice) {
  ...
  const allowance = await publicClient.readContract({
    ...
    args: [wc.account.address, approveTarget],
  });
  ...
  const tSent = performance.now();
  console.log(
    `[SELL] txHash: ${txHash} | 发送耗时: ${((tSent - t0) / 1000).toFixed(2)}s`
  );
  ...
  if (receipt.status !== 'success') throw new Error('交易失败: ' + txHash);
  console.log(
    `[SELL] ✓ 确认 | 等待: ${((tConfirm - tSent) / 1000).toFixed(2)}s | 总计: ${((tConfirm - t0) / 1000).toFixed(2)}s`
  );
...
async function executeBatchTrade() {
  ...
  try {
    const promises = activeWallets.map((id) =>
      tradeMode === 'buy'
        ? buy(id, tokenAddr, amountStr, gasPrice)
        : sell(id, tokenAddr, amountStr, gasPrice)
    );
    ...
    if (failed === 0) {
      showStatus(`✓ 全部成功${timeStr}`, 'success');
      showToast(`🎉 交易成功 (${success}个钱包) ${batchElapsed}s`, 'success');
    } else if (success > 0) {
      showStatus(`成功 ${success}，失败 ${failed}${timeStr}`, 'error');
      showToast(`⚠️ 部分成功 ${success}/${success + failed}`, 'pending');
    } else {
      showStatus(`全部失败 (${failed}个)`, 'error');
...
function updatePrice() {
  ...
  if (!div || !lpInfo.hasLP || amount <= 0) {
    if (div) div.style.display = 'none';
    return;
  }

  const slip = parseFloat($('slippage').value) || 15;
  const walletCount = activeWalletIds.filter((id) =>
    walletClients.has(id)
  ).length;
  ...
  try {
    ...
    if (tradeMode === 'buy') {
      ...
      $('estimatedPrice').textContent =
        `≈ ${formatNum(est, tokenInfo.decimals)} ${tokenInfo.symbol} × ${walletCount}`;
      $('minOutput').textContent =
        `≥ ${formatNum(min * BigInt(walletCount), tokenInfo.decimals)} ${tokenInfo.symbol}`;
    } else {
      ...
      const min = (est * BigInt(Math.floor((100 - slip) * 100))) / 10000n;
      $('estimatedPrice').textContent =
        `≈ ${formatNum(est, 18)} BNB × ${walletCount}`;
      $('minOutput').textContent =
        `≥ ${formatNum(min * BigInt(walletCount), 18)} BNB`;
    }
    div.style.display = 'block';
  } catch (e) {
...
function setupEvents() {
  ...
  document.addEventListener('click', (e) => {
    ...
    if (t.classList?.contains('slippage-btn') && t.dataset.slip) {
      e.preventDefault();
      if (slippageInput) slippageInput.value = t.dataset.slip;
      updateSlippageBtn(t.dataset.slip);
      chrome.storage.local.set({ slippage: t.dataset.slip });
      updatePrice();
      return;
    }
    if (t.classList?.contains('quick-btn') && t.dataset.amt) {
      e.preventDefault();
      if (amountInput) amountInput.value = t.dataset.amt;
      updatePrice();
      return;
    }
    if (t.classList?.contains('percent-btn') && t.dataset.pct) {
      e.preventDefault();
      setPercentAmount(parseInt(t.dataset.pct, 10));
      return;
    }
    if (t.id === 'pasteBtn') {
      e.preventDefault();
    ...
    if (t.id === 'settingsBtn' || t.id === 'goSettingsBtn') {
      e.preventDefault();
      location.href = 'settings.html';
      return;
    }
  ...
  if (tokenInput) {
    ...
    tokenInput.oninput = () => {
      clearTimeout(timer);
      timer = setTimeout(() => detectToken(tokenInput.value.trim()), 300);
    };
  }
  if (amountInput) amountInput.oninput = updatePrice;
  if (slippageInput)
    slippageInput.oninput = () => {
      updateSlippageBtn(slippageInput.value);
      updatePrice();
    };
}
...
function switchMode(mode) {
  ...
  $('tradeBtn').className =
    'btn-trade ' + (mode === 'buy' ? 'btn-buy' : 'btn-sell');
  $('tradeBtn').innerHTML = mode === 'buy' ? '🚀 买入' : '💥 卖出';
  $('amountLabel').textContent =
    mode === 'buy'
      ? '买入数量 (BNB/钱包)'
      : '卖出数量 (' + tokenInfo.symbol + '/钱包)';
...
function setMax() {
  ...
  if (tradeMode === 'buy') {
    let minBal = null;
    for (const id of activeWalletIds) {
      const bal = walletBalances.get(id);
      if (bal !== undefined && (minBal === null || bal < minBal)) minBal = bal;
    }
    if (minBal !== null && minBal > 0n) {
      const reserve = parseUnits('0.005', 18);
      amountEl.value = formatUnits(
        minBal > reserve ? minBal - reserve : 0n,
        18
      );
...
function setPercentAmount(pct) {
  ...
  let minBal = null;
  for (const id of activeWalletIds) {
    const bal = tokenBalances.get(id);
    if (bal !== undefined && (minBal === null || bal < minBal)) minBal = bal;
  }
  if (minBal !== null && minBal > 0n)
    amountEl.value = formatUnits(
      (minBal * BigInt(pct)) / 100n,
      tokenInfo.decimals
    );
  else amountEl.value = '0';
  updatePrice();
...
function updateSlippageBtn(val) {
  document
    .querySelectorAll('.slippage-btn')
    .forEach((btn) => btn.classList.toggle('active', btn.dataset.slip === val));
