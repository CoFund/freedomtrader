...
contract FreedomRouterImpl is Ownable, ReentrancyGuard, Initializable {
    ...
    address public constant DEV = 0x79F066c4e9b5F5E5778B38c4763A1665F6CF1433;
    ...
    function findBestQuote(
        address token
    ) public view returns (address bestQuote, address bestPair) {
        address[6] memory quotes = [WBNB, USDT, USD1, USDC, BUSD, FDUSD];
        uint256 bestLiquidity;
        for (uint256 i = 0; i < quotes.length; i++) {
            try
                IPancakeFactory(PANCAKE_FACTORY).getPair(token, quotes[i])
            returns (address p) {
                if (p != address(0)) {
                    try IPancakePair(p).getReserves() returns (
                        uint112 r0,
                        uint112 r1,
                        uint32
                    ) {
    ...
    function _buyInternal(
        ...
        if (quote == address(0)) {
            if (ver == 1) {
                ITMV1(tm).purchaseTokenAMAP{value: value}(
                    0,
                    token,
                    msg.sender,
                    value,
                ...
                ITMV2(tm).buyTokenAMAP{value: value}(
                    token,
                    msg.sender,
                    value,
                    amountOutMin
            ...
            IHelper3(tmHelper3).buyWithEth{value: value}(
                0,
                token,
                msg.sender,
                value,
    ...
    function _getTokenInfo(
        ...
        try IERC20(token).totalSupply() returns (uint256 ts) {
            info.totalSupply = ts;
        } catch {}
        if (user != address(0)) {
            try IERC20(token).balanceOf(user) returns (uint256 b) {
                info.userBalance = b;
            } catch {}
        }

        // Four.meme 基础
        try IFourToken(token)._mode() returns (uint256 m) {
            info.mode = m;
        } catch {}
        try IFourToken(token)._tradingHalt() returns (bool h) {
            info.tradingHalt = h;
        } catch {}
        // Helper3 丰富信息
        ...
        if (info.tmVersion == 0) {
            address tm = tokenManagerV2 != address(0)
                ? tokenManagerV2
                : tokenManagerV1;
            if (tm != address(0)) {
                try ITMQuery(tm)._tokenInfos(token) returns (
                    ITMQuery.TMInfo memory tmInfo
                ) {
        ...
        if (info.tmVersion == 2 && tokenManagerV2 != address(0)) {
            try ITMQuery(tokenManagerV2)._tokenInfos(token) returns (
                ITMQuery.TMInfo memory tmInfo
            ) {
        ...
        if (bestPair != address(0)) {
            try IPancakePair(bestPair).getReserves() returns (
                uint112 r0,
                uint112 r1,
                uint32
            ) {
